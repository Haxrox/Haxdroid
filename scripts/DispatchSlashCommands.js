...
(async () => {
  if (DISPATCH_COMMANDS) {
    ...
    const commandFiles = FileSystem.readdirSync(COMMANDS_PATH).filter((file) =>
      (file != 'Command.js' && file != 'SlashCommand.js'),
    );
    let command;
    commandFiles.filter((command) => true).forEach((file) => {
      if (file.endsWith('.js')) {
        command = require(`${RELATIVE_COMMANDS_PATH}/${file}`);
      } else {
        const commandFile = FileSystem.readdirSync(`${COMMANDS_PATH}/${file}`)
            .filter((subFile) =>
              subFile.endsWith('.js'),
            )
            .find((subFile) =>
              subFile.toUpperCase().startsWith(file.toUpperCase()),
            );
        command = require(`${RELATIVE_COMMANDS_PATH}/${file}/${commandFile}`);
      }
